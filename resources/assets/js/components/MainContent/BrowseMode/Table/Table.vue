    ...
    <table v-if="data.length !== 0">
      ...
      <tr v-if="data" class="table-row" v-for="row in data" :data-identifier="row[view.primaryKey]">
        ...
        <td v-for="(item, key) in row"
            ...
            :data-loc="`${$root.table.table}.${key}`"
            @contextmenu.prevent="dataModifier($event)"
            @click="seeCompleteData($event)"
            @keyup.ctrl.enter="resetFocus($event, true)">
...
export default {
  ...
  mounted() {
    ...
    this.$props.structure.forEach(databaseColumn => {
      if (databaseColumn.Key === "PRI") {
        this.view.primaryKey = databaseColumn.Field;
      }
    });
  },
  ...
  methods: {
     ...
     */
    resetFocus: function (ev, persistData) {
      if (this.view.cell.editing && persistData) {
